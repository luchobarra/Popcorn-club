...
export default function MovieRecommendations({
  ...
  const handleMoreInfo = (movieId: number) => {
    console.log(`Más información: ${movieId}`)
    // TODO: Navegar a /movie/${movieId} o abrir modal con detalles
  }

  const handleAddToWatchlist = (movieId: number) => {
    console.log(`Agregando a guardados: ${movieId}`)
    // TODO: Conectar con API para agregar a watchlist
    // Ejemplo: await addToWatchlist(movieId)
  }

  const handleAddToFavorites = (movieId: number) => {
    console.log(`Agregando a favoritos: ${movieId}`)
    // TODO: Conectar con API para agregar a favoritos
    // Ejemplo: await addToFavorites(movieId)
  }
    ...
    <div className="bg-primary min-h-screen py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
          ...
          {movies.map((movie) => (
            ...
            >
              ...
              <div className="p-4 space-y-2">
                {/* Title */}
                <h3 className="text-text-primary font-semibold text-base sm:text-lg leading-tight line-clamp-2 group-hover:text-secondary transition-colors duration-300">
                  {movie.title}
                </h3>
        ...
        <div className="text-center mt-8 sm:mt-12">
          <Button
            size="lg"
            className="bg-gradient-to-r from-secondary to-accent hover:from-secondary/90 hover:to-accent/90 text-text-primary font-semibold px-6 sm:px-8 py-3 rounded-xl shadow-lg hover:shadow-secondary/25 transition-all duration-300 transform hover:scale-105"
          >
